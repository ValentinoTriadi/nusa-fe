...
import { Shop } from '@/types/page/product';

export const ShopInformation = ({ shop }: { shop: Shop }) => {
  return (
    <div className="flex flex-col space-y-2 border-y border-gray-300 py-4">
      ...
      <div className="flex items-center space-x-4 text-sm">
        <div className="flex items-center space-x-1">
          ...
          <span className="text-gray-500">({shop.reviewCount} ulasan)</span>
        </div>
        <p>•</p>
        <div className="flex items-center space-x-1 text-gray-600">
          <MapPin className="h-4 w-4" />
          <span className="">
            {shop.location}, {shop.distance} km
          </span>
        </div>
      </div>
    </div>
